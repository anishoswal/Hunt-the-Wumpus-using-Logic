...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for i in range(len(tvec)):
        if tvec[i] != False:
            pr.append(proposition_bases_perceptual_fluents[i]+''+str(t))
        else:
            pr.append('~'+proposition_bases_perceptual_fluents[i]+''+str(t))
    axiom_str = ' & '.join(pr)
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    "*** YOUR CODE HERE ***"
    noPit =pit_str(x,y)
    noWumpus =wumpus_str(x,y)
    axiom_str = axiom_str + '~('+noPit + ' | '+noWumpus+')'
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    pitLoc = []
    axiom_str = axiom_str + breeze_str(x, y) + '<=> ('
    locations = [(x - 1, y), (x + 1, y), (x, y), (x, y - 1), (x, y + 1)]
    for (neighX, neighY) in locations:
        checkX = xmin <= neighX <= xmax
        checkY = ymin <= neighY <= ymax
        if checkX == True and checkY == True:
            pitLoc.append(pit_str(neighX, neighY))
    axiom_str = axiom_str + ' | '.join(pitLoc) + ')'
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    wumpusLoc = []
    axiom_str = axiom_str + stench_str(x, y) + '<=> ('
    locations = [(x - 1, y), (x, y - 1), (x + 1, y), (x, (y + 1))]
    for (neighX, neighY) in locations:
        checkX = xmin <= neighX <= xmax
        checkY = ymin <= neighY <= ymax
        if checkX == True and checkY == True:
            wumpusLoc.append(wumpus_str(neighX, neighY))
    axiom_str = axiom_str + ' | '.join(wumpusLoc) + ')'
    return axiom_str
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for xN in range(xmin, xmax):
        for yN in range(ymin, ymax):
            wumpusLoc = wumpus_str(xN, yN)
            wLocations.append(wumpusLoc)
    axiom_str = ' | '.join(wLocations)
    return axiom_str
    ...
    for x in range(xmin, xmax):
        for y in range(ymin, ymax):
            ...
            for xN in range(xmin, xmax):
                for yN in range(ymin, ymax):
                    if xN != x and yN != y:
                        noWumpus.append('('+wumpus_str(x, y)+' & '+wumpus_str(xN, yN)+')')
            wLocation.append('~ ('+' | '.join(noWumpus)+')')
    axiom_str = axiom_str + '('+' & '.join(set(wLocation))+')'
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    for xN in range(xmin, xmax):
        for yN in range(ymin, ymax):
            location = state_loc_str(xN, yN, t)
            loc.append(location) if (xN == xi) and (yN == yi) else loc.append('~' + location)
    axiom_str =axiom_str+' & '.join(loc)
    return axiom_str
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    for d in ['North', 'South', 'East', 'West']:
        headDir = 'Heading'+d+str(t)
        dir.append(headDir) if d.lower() == heading.lower() else dir.append('~'+headDir)
    axiom_str = axiom_str+' & '.join(dir)
    return axiom_str
...
def axiom_generator_have_arrow_and_wumpus_alive(t = 0):
    ...
    axiom_str = axiom_str+arrow+' & '+wumpus
    return axiom_str
...
def axiom_generator_location_OK(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = state_OK_str(x,y,t)+' <=> ('+'~'+pit_str(x,y)+' & (' + '~' + wumpus_str(x,y)+' | (' + wumpus_str(x,y)+' & ~' + state_wumpus_alive_str(t)+'))'+')'
    return axiom_str
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = axiom_str+'('+state_loc_str(x, y, t)+')'+' >> '
    axiom_str = axiom_str+'('+percept_breeze_str(t)+' >> '+breeze_str(x, y)+' & '+breeze_str(x, y)+' >> '+percept_breeze_str(t)+')'
    return axiom_str
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    "*** YOUR CODE HERE ***"
    axiom_str =axiom_str+'('+state_loc_str(x, y, t)+')'+' >> '
    axiom_str = axiom_str+'('+percept_stench_str(t)+' >> '+stench_str(x, y)+' & '+stench_str(x, y)+' >> '+percept_stench_str(t)+')'
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    if xmin <= x-1 <= xmax and ymin <= y <= ymax:
        loc.append(state_loc_str(x - 1, y, t))
    if xmin <= x+1 <= xmax and ymin <= y <= ymax:
        loc.append(state_loc_str(x + 1, y, t))
    if xmin <= x <= xmax and ymin <= y-1 <= ymax:
        loc.append(state_loc_str(x, y - 1, t))
    if xmin <= x <= xmax and ymin <= y+1 <= ymax:
        loc.append(state_loc_str(x, y + 1, t))
    axiom_str = axiom_str+'('+' | '.join(loc) + ')'+' & '+action_forward_str(t)+')'+' | '+'~ ('+'~'+state_loc_str(x, y, t)+' | '+action_forward_str(t)+'))'
    return axiom_str
...
def axiom_generator_have_arrow_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = state_have_arrow_str(t+1)+' <=> '+'('+'~'+action_shoot_str(t)+' & '+state_have_arrow_str(t)+')'
    return axiom_str
...
def axiom_generator_heading_north_ssa(t):
    ...
    axiom_str = axiom_str+' | ('+state_heading_east_str(t)+' & '+action_turn_left_str(t)+') | ('+state_heading_west_str(t)+' & '+action_turn_right_str(t)+'))'
    return axiom_str
...
def axiom_generator_heading_east_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = state_heading_east_str(t + 1) + ' <=> ('+'('+ state_heading_east_str(
        t) + ' & ~('+action_turn_right_str(t)+' | '+action_turn_left_str(t)+')'+')'
    axiom_str = axiom_str + ' | (' + state_heading_south_str(t) + ' & ' + action_turn_left_str(
        t) + ') | (' + state_heading_north_str(t) + ' & ' + action_turn_right_str(t) + '))'
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = state_heading_south_str(t + 1) + ' <=> ('+'(' + state_heading_south_str(
        t) + ' & ~('+action_turn_right_str(t)+' | '+action_turn_left_str(t)+')'+')'
    axiom_str = axiom_str + ' | (' + state_heading_west_str(t) + ' & ' + action_turn_left_str(
        t) + ') | (' + state_heading_east_str(t) + ' & ' + action_turn_right_str(t) + '))'
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = state_heading_west_str(t + 1) + ' <=> ('+'(' + state_heading_west_str(
        t) + ' & ~('+action_turn_right_str(t)+' | '+action_turn_left_str(t)+')'+')'
    axiom_str = axiom_str + ' | (' + state_heading_north_str(t) + ' & ' + action_turn_left_str(
        t) + ') | (' + state_heading_south_str(t) + ' & ' + action_turn_right_str(t) + '))'
    return axiom_str
    return axiom_str
...
def axiom_generator_heading_only_east(t):
    ...
    "*** YOUR CODE HERE ***"
    # Comment or delete the next line once this function has been implemented.
    axiom_str = state_heading_east_str(t) + ' <=> ~(' + state_heading_south_str(t) + ' | ' + state_heading_west_str(t) + ' | ' + state_heading_north_str(t) + ')'
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_only_west(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = state_heading_west_str(t) + ' <=> ~(' + state_heading_south_str(t) + ' | ' + state_heading_north_str(t) + ' | ' + state_heading_east_str(t) + ')'
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    action = []
    t = str(t)
    for act1 in proposition_bases_actions:
        notActions = []
        for act2 in proposition_bases_actions:
            if act1 == act2:
                pass
            else:
                notActions.append(act2+t)
        action.append('('+act1+t+' <=> ('+'~('+' | '.join(notActions)+'))'+')')
